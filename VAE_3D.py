...
class Encoder(nn.Module):
    ...
    def __init__(self, channels, channel_intermed=12, ch=64, blocks=(1, 2, 4, 8), latent_channels=256, image_shape=(1, 128, 192, 128), num_res_blocks=1, norm_type="bn",
                 deep_model=False):
        ...
        self.final_conv = nn.Conv3d(widths_out[-1] * ch, channel_intermed, 1, 1) 
        
        self.intermediate_shape = np.array(image_shape) // (2**len(blocks))
        self.intermediate_shape[0] = channel_intermed
        
        self.fc_mu = nn.Linear(int(np.prod(self.intermediate_shape)), int(latent_channels))
        self.fc_log_var = nn.Linear(int(np.prod(self.intermediate_shape)), int(latent_channels))
...
class Decoder(nn.Module):
    ...
    def __init__(self, channels, channel_intermed=12, ch=64, blocks=(1, 2, 4, 8), latent_channels=256, image_shape=(1, 128, 192, 128), num_res_blocks=1, norm_type="bn",
        ...
        self.fc_in = nn.Sequential(
            nn.Linear(latent_channels+2, int(np.prod(self.intermediate_shape))),
        ...
        self.conv_in = ds_Conv3d(channel_intermed, widths_in[0] * ch, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, ventricle_vol, brain_vol):
        x = torch.cat((x, ventricle_vol.unsqueeze(1), brain_vol.unsqueeze(1)), dim=1)
...
class VAE(nn.Module):
    ...
    """
    def __init__(self, channel_in=1, channel_intermed=24, ch=64, blocks=(1, 2, 4, 8), image_shape=(1, 128, 192, 128), latent_channels=256, num_res_blocks=1, norm_type="bn",
                 deep_model=False):
        ...
        self.encoder = Encoder(channel_in, channel_intermed=channel_intermed, ch=ch, blocks=blocks, latent_channels=latent_channels, image_shape=image_shape,
                               num_res_blocks=num_res_blocks, norm_type=norm_type, deep_model=deep_model)
        self.decoder = Decoder(channel_in, channel_intermed=channel_intermed,ch=ch, blocks=blocks, latent_channels=latent_channels, image_shape=image_shape,
                               num_res_blocks=num_res_blocks, norm_type=norm_type, deep_model=deep_model)
